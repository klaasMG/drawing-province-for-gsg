...
class MyDrawWindow(QGraphicsView):
    ...
    def mouseReleaseEvent(self , event):
        if self.tool == ("free hand" or "erase"):
            if event.button() == Qt.LeftButton:
                self.using_tool = False
            elif event.button() == Qt.RightButton:
                self.using_tool = False
            ...
            try:
                ...
                if tool == "free hand":
                    ...
                    if len(points) == 2:
                        ...
                        point1 = points[0]
                        point2 = None
                    tool, data = self.free_hand(tool,point1,point2,0)
    ...
    def run(self):
        while True:
            ...
            if tool == "free hand":
                ...
                red, green, bleu = colour
                alpha = 0
                self.draw_image.putpixel(point1,(red, green, bleu, alpha))
                if point2 is not None:
                    draw = ImageDraw.Draw(self.draw_image)
                    draw.line((point1[0] , point1[1] , point2[0] , point2[1]) , fill=(red , green , bleu) , width=1)
            elif tool == "save":
